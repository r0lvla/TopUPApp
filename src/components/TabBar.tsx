    ...
    <>
      <div
        style={{
          ...
          height: 40,
          background: 'linear-gradient(to top, rgba(28,28,30,0.7) 0%, transparent 100%)',
      ...
      >
        {TABS.map((tab) => {
          ...
          return (
            <button
              key={tab.id}
              onClick={() => { if (active !== tab.id) { selection(); onChange(tab.id); } }}
              style={{
            ...
            >
              ...
              <span style={{ fontSize: 10, fontWeight: isActive ? 600 : 500, letterSpacing: -0.1, color: 'inherit' }}>
                {tab.label}
              </span>
            </button>
          );
